...
class QuoteWidgetLarge : AppWidgetProvider() {
    companion object {
        private const val REFRESH_ACTION = "com.mywritting.REFRESH_WIDGET_LARGE"
        private const val TAG = "QuoteWidgetLarge"
    }
    ...
    override fun onReceive(context: Context, intent: Intent) {
        try {
            ...
            if (intent.action == REFRESH_ACTION) {
                ...
                val appWidgetIds = appWidgetManager.getAppWidgetIds(ComponentName(context, QuoteWidgetLarge::class.java))
    ...
    private fun updateAppWidget(
        ...
        try {
            ...
            // Create RemoteViews
            val views = RemoteViews(context.packageName, R.layout.quote_widget_large)
            
            try {
                // Set up refresh button click intent
                val refreshIntent = Intent(context, QuoteWidgetLarge::class.java).apply {
            ...
            try {
                ...
                if (openAppIntent != null) {
                    val openAppPendingIntent = PendingIntent.getActivity(
                        context,
                        appWidgetId + 1000, // Use different request code from small widget
                        openAppIntent,
                        PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
                    )
